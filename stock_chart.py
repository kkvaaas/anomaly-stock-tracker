...
class StockChartGenerator:
    ...
    def generate_line_chart(self, ticker: str, prices: List[float], times: List[datetime]) -> io.BytesIO:
        ...
        try:
            ...
            plt.plot(dates[0], start_price, 'go', markersize=8, label=f'Начало: {start_price:.2f}')
            
            # Точка конца (красная)
            end_price = prices[-1]
            plt.plot(dates[-1], end_price, 'ro', markersize=8, label=f'Текущая: {end_price:.2f}')
    ...
    async def update_user_charts(self, chat_id: str) -> None:
        ...
        for ticker in user["stocks"]:
            try:
                ...
                if not prices:
                    print(f"Нет данных для {ticker} с {registration_date.strftime('%d.%m.%Y')}")
                    continue
